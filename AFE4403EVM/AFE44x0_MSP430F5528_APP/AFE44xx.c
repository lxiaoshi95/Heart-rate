...
#ifdef __TX3_MODE__
#ifdef __CT__
  AFE44xx_Reg_Write((unsigned char)CONTROL3, (TX3_MODE + CLK_DIV_7));    //0x31
#else
  AFE44xx_Reg_Write((unsigned char)CONTROL3, TX3_MODE);    //0x31
#endif
#else
#ifdef __CT__
  AFE44xx_Reg_Write((unsigned char)CONTROL3, CLK_DIV_7);    //0x31
#else
...
unsigned long AFE44xx_Reg_Read(unsigned char Reg_address)
{
  ...
  union Raw2Long raw_signal;
  
  raw_signal.SPI_Rx_buf[3] = 0x00;
  P4OUT&= ~0x01;   //  SEN LOW FOR TRANSMISSION.
  // Loop unrolling for machine cycle optimization
  UCB1TXBUF = Reg_address;                    // Send the first byte to the TX Buffer: Address of register
  while ( (UCB1STAT & UCBUSY) );		// USCI_B1 TX buffer ready?
  raw_signal.SPI_Rx_buf[2] = UCB1RXBUF;			// Read Rx buf
  UCB1TXBUF = 0;                              // Send the second byte to the TX Buffer: dummy data
  while ( (UCB1STAT & UCBUSY) );		// USCI_B1 TX buffer ready?
  raw_signal.SPI_Rx_buf[2] = UCB1RXBUF;			// Read Rx buf: Data[23:16]
  UCB1TXBUF = 0;                              // Send the third byte to the TX Buffer: dummy data
  while ( (UCB1STAT & UCBUSY) );		// USCI_B1 TX buffer ready?
  raw_signal.SPI_Rx_buf[1] = UCB1RXBUF;			// Read Rx buf: Data[15:8]
  UCB1TXBUF = 0;                              // Send the first byte to the TX Buffer: dummy data
  while ( (UCB1STAT & UCBUSY) );		// USCI_B1 TX buffer ready?
  raw_signal.SPI_Rx_buf[0] = UCB1RXBUF;			// Read Rx buf: Data[7:0]
  P4OUT|=0x01;  // set HIGH at end of transmission
  return raw_signal.retVal;
}
